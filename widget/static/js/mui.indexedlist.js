	...
	var IndexedList = $.IndexedList = $.Class.extend({
		 ...
		 **/
		 ...
		 init:function(holder, options){
			var self = this;
			mui.ajax('static/db/area.json', { //这个是数据导入的ajax  填入url  即可
                data: {},
                dataType: 'json', //服务器返回json格式数据
                type: 'get', //HTTP请求类型,必须为get
                ...
                success: function(data) {
                    ...
                    data.forEach(function(item) {
                        if (item.cities && item.cities.length > 0) {
                            itemhtml += "<li data-group="+item.name+" class='mui-table-view-divider mui-indexed-list-group'>"+item.name+"</li>";
                            for (var k = 0; k < item.cities.length; k++) {
                                itemhtml += "<li data-value="+item.cities[k].cityid+" data-tags="+item.cities[ k].tags+" class='mui-table-view-cell mui-indexed-list-item'>"+item.cities[k].name+"</li>"
                            }
                        }
                    })
                    //table.insertBefore(itemhtml, table.childNodes[0]);  //插入热门城市
